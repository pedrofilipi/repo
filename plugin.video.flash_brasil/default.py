...
def list_items_by_group(group_title, content_type):
    ...
    if content_type == 'movie':
        ...
        for item in items:
            add_video_item(item, is_series=False)
    else:
...
def add_video_item(item, is_series=False):
    ...
    if is_series:
        url = get_url(action='play_series', path=item['path'], title=item['title'])
    else:
        # Para filmes, usamos o play normal que não cria playlist
        url = get_url(action='play_movie', path=item['path'])
...
def list_episodes(show_name, group_title):
    ...
    if group_title in series:
        ...
        for item in episodes:
            add_video_item(item, is_series=True)
...
def play_series(path, title):
    title = to_unicode(title)
    data = get_data()
    series = data['series']
    current_show = get_clean_show_name(title)
    ...
    for group in series.values():
        for item in group:
            item_title = to_unicode(item['title'])
            if not found_current and item_title == title:
                found_current = True
            
            if found_current:
                if get_clean_show_name(item_title) == current_show:
                    li = xbmcgui.ListItem(to_utf8(item_title))
                    li.setInfo('video', {'title': to_utf8(item_title)})
...
def play_movie(path):
    # Para filmes, limpamos a playlist e tocamos apenas um item
    playlist = xbmc.PlayList(xbmc.PLAYLIST_VIDEO)
    playlist.clear()
...
def router(paramstring):
    ...
    if params:
        ...
        if action == 'list_movies':
            ...
            play_movie(params.get('path'))
        elif action == 'play_series':
            play_series(params.get('path'), params.get('title'))
